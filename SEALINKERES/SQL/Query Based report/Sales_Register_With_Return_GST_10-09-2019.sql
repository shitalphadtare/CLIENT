...
(Case When T0.Doctype = 'I' Then 
   (CASE When ITM.ItemClass = 1 Then (Select ServCode from OSAC Where AbsEntry = (case when  t1.sacentry is  null then  ITM.SACEntry else t1.sacentry end))  
         When ITM.ItemClass  = 2 Then (select ChapterID  from ochp where Absentry=(case when  t1.HsnEntry is  null then  ITM.chapterid else t1.HsnEntry end)) End)       
