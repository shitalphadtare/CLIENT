...
select dln.DocEntry,
					...
					isnull(	(CASE WHEN LEN(dn1.U_itemdesc3)-LEN(REPLACE(dn1.U_itemdesc3, ' ', '')) < 10 
...
(SUBSTRING(dn7.U_ITEMDESC, 0, CHARINDEX(' ', dn7.U_ITEMDESC,CHARINDEX(' ',dn7.U_ITEMDESC,CHARINDEX(' ',dn7.U_ITEMDESC,CHARINDEX(' ',dn7.U_ITEMDESC,CHARINDEX(' ',dn7.U_ITEMDESC,
					...
					,case when dln.U_tcaddrs='Bill To' then  dln.PayToCode
							...
							,cast(fv.U_Start_Qty as varchar)+'-'+cast(fv.U_End_Qty as varchar) 'Mark_No'
	...
	order by cast(dln.U_DFVNum  as int),dln.DocEntry,dn1.VisOrder asc
