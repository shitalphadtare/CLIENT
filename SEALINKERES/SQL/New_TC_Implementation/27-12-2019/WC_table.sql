CREATE view [dbo].[WC_table] as
select dln.DocEntry,dn1.VisOrder,dn1.Quantity,cd3.U_Mark_No,
(case when isnull((select max(U_End_qty) from [@WC_D] where U_Mark_no=CD4.U_mark_no),0)=0 then isnull(CD4.U_mark_qty,0)
else (select max(U_End_qty) from [@WC_D] where U_Mark_no=CD4.U_mark_no) end)+1 'Start_Quantity'
,dln.cardcode
,cd3.U_mark_qty
from ODLN dln 
