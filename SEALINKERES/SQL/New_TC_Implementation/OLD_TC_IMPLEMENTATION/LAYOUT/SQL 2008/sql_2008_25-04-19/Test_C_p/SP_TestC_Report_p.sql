...
alter Procedure [dbo].[SP_TestC_Report_p]
as
begin

;with CTE_RunningTotal as
		( select number,quantity 'quantity',county,docentry,visorder,quantity1 
		from
		(select row_number() over (PARTITION BY b,COUNTY order by docentry,visorder ) 'number',* 
								...
								where dln.U_DTCWCReq='TC' and dn1.U_rtcreq='YES' and dln.CANCELED<>'Y' and dn1.TargetType<>16 and dln.CANCELED<>'Y'
								)A
		)b),
-----------------------------------------------------------------------------			
	TestC_Report_p as(select dln.DocEntry,
							 dln.CardName,
							 dln.DocNum,
							 dln.DocDate,
							...
							dn1.ItemCode,
							isnull((SUBSTRING(dn1.U_ItemDesc3, 0, CHARINDEX(' ', dn1.U_ItemDesc3, CHARINDEX(' ', dn1.U_ItemDesc3, CHARINDEX(' ', dn1.U_ItemDesc3, 0)+1)+1)))
							,(SUBSTRING(dn1.Dscription, 0, CHARINDEX(' ', dn1.Dscription, CHARINDEX(' ', dn1.Dscription, CHARINDEX(' ', dn1.Dscription, 0)+1)+1)))) 'Dscription',
							dn1.Quantity,
...
select cast(cast((case when Mark_Qty=1 then Mark_Qty else Mark_Qty+1-Quantity end) as int) as varchar)+'-'+cast(cast(Mark_Qty as int) as varchar) 'Mark_No'
		,*		
		from 
		(select (select sum(Quantity) from CTE_RunningTotal c where c.number<=b.number and isnull(c.County,'')=isnull(b.County,''))+b.quantity1 'Mark_Qty'
					,b.number,b.Quantity 'B_Qty',b.County 'B_County',b.VisOrder 'B_Vis',a.*
				from TestC_Report_p A
