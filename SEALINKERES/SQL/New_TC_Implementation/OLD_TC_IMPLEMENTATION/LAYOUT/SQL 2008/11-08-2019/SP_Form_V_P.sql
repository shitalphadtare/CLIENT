...
(select number,quantity,county,docentry,visorder,cardcode from
                                                                                                                (select row_number() over (PARTITION BY b,COUNTY
                                                                                                                ,cardcode ---added on 08-08-2019
                                                                                                                order by docentry,visorder ) 'number',* from 
                                                                                                                                ...
                                                                                                                                ,dn1.VisOrder,
                                                                                                                                dn1.DocEntry,cd3.County,dln.cardcode
                                                                                                                                from ODLN dln 
...
Form_V_P as(select dln.DocEntry,
                                                                                ...
                                                                                isnull(                     
                                                                                (CASE WHEN LEN(dn1.U_itemdesc3)-LEN(REPLACE(dn1.U_itemdesc3, ' ', '')) < 10 
            THEN dn1.U_itemdesc3
            ELSE
...
(SUBSTRING(dn1.U_itemdesc3, 0, CHARINDEX(' ', dn1.U_itemdesc3,CHARINDEX(' ',dn1.U_itemdesc3,CHARINDEX(' ',dn1.U_itemdesc3,CHARINDEX(' ',dn1.U_itemdesc3,CHARINDEX(' ',dn1.U_itemdesc3,
                                                                                                                                ...
                                                                                                                                +1)+1)+1)+1)+1)+1)+1)+1))) end),   
                                                                                                                                (CASE WHEN LEN(dn1.Dscription)-LEN(REPLACE(dn1.Dscription, ' ', '')) < 10 
            THEN cast(dn1.Dscription AS nvarchar(max))
            ELSE                                                                                                            
...
(SUBSTRING(dn7.U_ITEMDESC, 0, CHARINDEX(' ', dn7.U_ITEMDESC,CHARINDEX(' ',dn7.U_ITEMDESC,CHARINDEX(' ',dn7.U_ITEMDESC,CHARINDEX(' ',dn7.U_ITEMDESC,CHARINDEX(' ',dn7.U_ITEMDESC,
                                                                                                                                ...
                                                                                                                                +1)+1)+1)+1)+1)+1)+1)+1))) end 'Dscription',
                                                                                dn1.Quantity,
                                                                                ...
                                                                                ,case when dln.U_tcaddrs='Bill To' then  dln.PayToCode
                                                                                                                else dln.ShipToCode end 'THIRD_PARTY',dln.cardcode
                                                from ODLN dln 
                                                Inner join DLN1 dn1 on dln.DocEntry=dn1.DocEntry
...
-------------------------------------------------------------------------------------------------------------------------------------------
                                ...
                                ,*                             
                                from 
                                (select (select sum(Quantity) from CTE_RunningTotal c where c.number<=b.number and isnull(c.County,'')=isnull(b.County,'')
                                and c.CardCode=A.CardCode--added on 08082019
                                )++isnull((select U_markno from ocrd where cardcode=a.cardcode and  GETDATE()<'20200401'),0)'Mark_Qty'
                                                                                ,b.number,b.Quantity 'B_Qty',b.County 'B_County',b.VisOrder 'B_Vis',a.*
                                                                from Form_V_P A
                                                                left outer join CTE_RunningTotal B on a.DocEntry=b.DocEntry and a.VisOrder=b.VisOrder-- and a.cardcode=b.cardcode
                                ) final
