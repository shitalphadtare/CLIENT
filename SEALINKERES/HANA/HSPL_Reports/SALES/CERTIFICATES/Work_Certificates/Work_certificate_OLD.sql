CREATE view Work_certificate_OLD as
...
dn1."ItemCode", IFNULL(dn1."U_ItemDesc3", dn1."Dscription") AS "Dscription", dn1."Quantity", dn1.U_TCDT AS "TEST DATE", dn1.U_PROOFLOAD AS "PROOF LOAD", 
dn1.U_SWL AS "SWL", '' AS "DISTING MARK", '' AS "TYPE OF", '' AS "SERIAL NO", dn1.U_LIFT AS "LIFT", 0 AS "Item Quanitity", dn1."VisOrder", '' AS "ItmsGrpNam", 
...
WHERE CANCELED <> 'Y' AND dln1."CardCode" = dln."CardCode" AND DLN1."DocEntry" < dln."DocEntry" AND CD2."County" = CD3."County" AND
 year(NOW()) = year(DLN1."DocDate") AND dln1."U_DTCWCReq" = 'WC') + 
 IFNULL((SELECT CASE WHEN crd."CardFName" = cd1."County" THEN crd."U_WCMarkNo" ELSE cd1."U_WC_Qty" END AS "a"
  FROM ODLN dln1 LEFT OUTER JOIN OCRD crd ON dln1."CardCode" = crd."CardCode" 
  LEFT OUTER JOIN CRD1 cd1 ON dln1."CardCode" = cd1."CardCode" AND dln1."ShipToCode" = cd1."Address" AND cd1."AdresType" = 'S'
   WHERE dln1."DocEntry" = dln."DocEntry" AND NOW() < '20200101'), 0) AS "start_quantity", 
   ...
   dln."TaxDate", IFNULL((SELECT "U_TCPrefix" FROM "@TC_NUM" WHERE "Code" = 'WC'), '') || dln."U_DWCNum" AS "TC Number", dln.U_SIGN, 
