CREATE view TestC_Report_New
as

SELECT dln."DocEntry", dln."CardName", dln."DocDate", 
...
dn1."ItemCode", IFNULL(dn1."U_ItemDesc3", dn1."Dscription") AS "Dscription", 
dn1."Quantity", dn1.U_TCDT AS "TEST DATE", dn1.U_PROOFLOAD AS "PROOF LOAD", 
dn1.U_SWL AS "SWL", '' AS "DISTING MARK", '' AS "TYPE OF", '' AS "SERIAL NO", 
...
dln."TaxDate", dln.U_SIGN, 
CASE WHEN year(dln."DocDate") = '2019' THEN IFNULL((SELECT "U_TCPrefix" FROM "@TC_NUM" WHERE "Code" = 'TC'), '') 
ELSE (SELECT "U_TCPrefix" FROM "@TC_NUM" WHERE right("Code", 4) = CAST(year(dln."DocDate") AS varchar) AND left("Code", 2) = 'TC') END ||
 dln."U_DTCNum" AS "TC Number", 
