...
DN7.U_TCDT AS "TEST DATE", dn7.U_PRROFLOAD AS "PROOF LOAD", DN7.U_SWL AS "SWL", '' AS "DISTING MARK", '' AS "TYPE OF", 
...
(SELECT IFNULL(SUM(IFNULL("Quantity", 0)), 0) FROM ODLN dln1 INNER JOIN dln1 dn2 ON DLN1."DocEntry" = dn2."DocEntry" 
	LEFT OUTER JOIN CRD1 CD2 ON DLN1."ShipToCode" = CD2."Address" AND "AdresType" = 'S' WHERE CANCELED <> 'Y' 
	AND dln1."CardCode" = dln."CardCode" AND DLN1."DocEntry" < dln."DocEntry" AND CD2."County" = CD3."County" AND 
	year(NOW()) = year(DLN1."DocDate") AND dln1."U_DTCWCReq" = 'TC' AND dn2."TargetType" <> 16 AND dn2."U_RTCReq" = 'Yes') + 
...
dln."TaxDate", dln.U_SIGN, IFNULL((SELECT "U_TCPrefix" FROM "@TC_NUM" WHERE "Code" = 'TC'), '') || dln."U_DTCNum" AS "TC Number", 
